...
function Management() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (!categoriasRes.ok || !ingredientesRes.ok || !produtosRes.ok || !estoquesRes.ok) {
  ...
  const handleCreateOrUpdate = async (entity, data, endpoint, isEdit = false) => {
    try {
      ...
      if (!response.ok) throw new Error(`Erro ao ${isEdit ? 'alterar' : 'criar'} ${entity}`);
      alert(`${entity} ${isEdit ? 'alterado' : 'criado'} com sucesso!`);
      fetchData();
      setNovoProduto({    id: null,
        nome: '',
        descricao: '',
        valor: '',
        categoriaId: '',
          ...
          <main className="management-container">
            ...
            <section>
              ...
              <form
                onSubmit={(e) => {
                  e.preventDefault();
                  const endpoint = `http://localhost:8080/${novoProduto.id ? 'estoque/produto/update' : 'cardapio/produto/create'}`;
                  handleCreateOrUpdate('Produto', novoProduto, endpoint, !!novoProduto.id);
                }}
              >
                <input
                  type="text"
                ...
                <div className="form-group">
                  ...
                  <Select
                    ...
                    isMulti
                    onChange={(option) => setNovoProduto({ ...novoProduto, ingredientes:  option.map((opt) => (opt.value))})
                  }
                  />
                </div>
                <select
                  value={novoProduto.estoqueId}
                  onChange={(e) => setNovoProduto({ ...novoProduto, estoqueId: e.target.value })}
                >
                ...
                <button type="submit">{novoProduto.id ? 'Alterar' : 'Criar'}</button>
              </form>
              <ul>
                {produtos.map((prod) => (
                  <li key={prod.id}>
            ...
            <section>
              ...
              <form
                onSubmit={(e) => {
                  e.preventDefault();
                  const endpoint = `http://localhost:8080/estoque/ingrediente/${novoIngrediente.id ? 'update' : 'create'}`;
                  handleCreateOrUpdate('Ingrediente', novoIngrediente, endpoint, !!novoIngrediente.id);
                }}
              >
                <input
                  type="text"
                ...
                <input
                  ...
                  onChange={(e) => setNovoIngrediente({ ...novoIngrediente, descricao: e.target.value })}
                />
                <input
                  type="number"
                  placeholder="Quantidade"
                ...
                <select
                  ...
                  onChange={(e) => setNovoIngrediente({ ...novoIngrediente, estoqueId: e.target.value })}
                  >
                <option value="">Selecione o Estoque</option>
                  {estoques.map((est) => (
                    <option key={est.id} value={est.id}>
              ...
              <button type="submit">{novoIngrediente.id ? 'Alterar' : 'Criar'}</button>
            </form>
            <ul>
              {ingredientes.map((ing) => (
                <li key={ing.id}>
                  {ing.nome} - {ing.quantidade} {ing.unidade}
                  <button onClick={() => setNovoIngrediente(ing)}>Alterar</button>
                  <button
                    onClick={() =>
                      handleDelete('Ingrediente', ing.id, 'http://localhost:8080/estoque/ingrediente/delete')
                      }
                    >
                      Excluir
                    </button>
